...
pebble_deck packs eight console lines onto the screen. deck_lite keeps the same data sources and the same
...
```
pebble build
pebble install --phone <phone-ip>      # or: pebble install --emulator emery
```

The face targets `emery` (Pebble Time 2, 200×228) like the original. It uses its own UUID and settings storage,
so it installs alongside pebble_deck rather than replacing it.

## Credits

All the data plumbing (weather via Open-Meteo, altitude, custom API, HRV RMSSD measurement, compass
power management) is keatr0n's work from pebble_deck; this fork only changes what is drawn on the screen.
